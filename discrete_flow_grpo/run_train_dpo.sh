#!/bin/bash
# =============================================================================
# On-policy DPO Training (Bradley-Terry over discrete-flow trajectories)
# =============================================================================
# Loss:  -log σ( (β'/(K-1)) Σ_k [ log π*(x⁺_{t_k}|x⁺_{t_{k-1}})/π_old(...)
#                                - log π*(x⁻_{t_k}|x⁻_{t_{k-1}})/π_old(...) ] )
#
# Usage:
#   bash run_train_dpo.sh                              # defaults: pickscore reward
#   REWARD='{"geneval": 1}' bash run_train_dpo.sh      # geneval reward (auto → geneval dataset)
#   RUN_TAG=exp01 bash run_train_dpo.sh                # custom run name
#   NUM_GPUS=4 bash run_train_dpo.sh                   # 4 GPUs
#   RESUME_STEP=500 bash run_train_dpo.sh              # resume from step 500
#   USE_CFG=1 bash run_train_dpo.sh                    # enable CFG
#   BETA_DPO=500 bash run_train_dpo.sh                 # DPO temperature β'
# =============================================================================
...
# DATASET auto-detection: only pickscore + geneval supported in DPO
if [ -z "${DATASET:-}" ]; then
...
BETA_DPO=${BETA_DPO:-100}                         # DPO temperature β'
DPO_NUM_PAIRS=${DPO_NUM_PAIRS:-1}                 # 0 = G//2 pairs (top vs bottom)
...
USE_CFG=${USE_CFG:-0}                             # 0 = no CFG (--no_cfg); 1 = enable CFG
NUM_GPUS=${NUM_GPUS:-8}                           # number of GPUs
EXTRA_ARGS=${EXTRA_ARGS:-}                        # extra CLI flags for train_DPO.py
SAMPLE_BATCH_SIZE=24
TRAIN_BATCH_SIZE=1       # number of PAIRS per forward (=> 2× samples in model)
# =============================================================================
...
if [ -n "$RUN_TAG" ]; then
    OUTPUT_NAME="grpo_dpo_${REWARD_TAG}_${RUN_TAG}"
else
    OUTPUT_NAME="grpo_dpo_${REWARD_TAG}"
fi
_NO_CFG_FLAG="--no_cfg"
...
echo "  β' (DPO):   ${BETA_DPO}"
echo "  Pairs:      ${DPO_NUM_PAIRS} (0 = G//2)"
