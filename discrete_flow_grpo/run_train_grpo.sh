#!/bin/bash
# =============================================================================
# GRPO Training (token-level normalized)
# =============================================================================
# Usage:
#   bash run_train_grpo.sh                              # defaults: pickscore reward, pickscore dataset
#   REWARD='{"aesthetic": 1}' bash run_train_grpo.sh    # aesthetic reward (auto → pickscore dataset)
#   REWARD='{"geneval": 1}' bash run_train_grpo.sh      # geneval reward (auto → geneval dataset)
#   REWARD='{"ocr": 1}' bash run_train_grpo.sh          # ocr reward (auto → ocr dataset)
#   DATASET=drawbench REWARD='{"clip": 1}' bash run_train_grpo.sh  # manual dataset override
#   RUN_TAG=exp01 bash run_train_grpo.sh                # custom run name
#   NUM_GPUS=4 bash run_train_grpo.sh                   # 4 GPUs
#   RESUME_STEP=500 bash run_train_grpo.sh              # resume from step 500
#   USE_CFG=1 bash run_train_grpo.sh                    # enable CFG (appends _cfg to name)
#
# Naming convention:
#   Output dir: output_grpo_<REWARD_TAG>[_<RUN_TAG>]/
#   Log file:   logs/grpo_<REWARD_TAG>[_<RUN_TAG>].log
#   e.g.        output_grpo_pickscore/
#               output_grpo_geneval_exp01/
# =============================================================================
...
RUN_TAG=${RUN_TAG:-}                            # custom suffix for output dir name
...
EXTRA_ARGS=${EXTRA_ARGS:-}                      # extra CLI flags for train_GRPO.py
...
# Extract reward tag: '{"pickscore": 1}' -> pickscore, '{"aesthetic": 0.5, "clip": 0.5}' -> aesthetic_clip
REWARD_TAG=$(echo "$REWARD" | python3 -c "import sys,json; d=json.load(sys.stdin); print('_'.join(sorted(d.keys())))")
if [ -n "$RUN_TAG" ]; then
    OUTPUT_NAME="grpo_${REWARD_TAG}_${RUN_TAG}"
else
    OUTPUT_NAME="grpo_${REWARD_TAG}"
fi
...
if [ "${RESUME_STEP}" -gt 0 ] 2>/dev/null; then
    # Search for checkpoint_*_step{RESUME_STEP} inside OUTPUT_DIR
...
# Rewards that benefit from GPU: pickscore, aesthetic, clip, imagereward, ocr, qwenvl, etc.
# Rewards that do NOT benefit (HTTP/CPU-only): geneval, jpeg, deqa, unifiedreward
_GPU_REWARD_FLAG=""
...
if not set(d.keys()).issubset(cpu_only):
    sys.exit(0)  # has at least one GPU-benefiting reward
sys.exit(1)
"; then
